...
function mapRefs(){
  ...
  "data","inicio","fim","linha","op","motivo","obs","chars","suporte","statusForm","btnSalvar","btnEncerrar","btnApagar","btnRestaurar","genericModal","modalTitle","modalText","modalContent","modalCancel","modalOk"]
...
async function init(){
  ...
  if(!CONFIG.supabaseUrl || CONFIG.supabaseUrl.includes("COLE_AQUI")){
    setStatus("Configure web/js/config.js com URL e anon key do Supabase.", "error");
    showLogin();
    return;
  }
  if(!window.supabase){
...
function bindEvents(){
  ...
  refs.btnSetupAdmin.onclick = setupAdminMaster;
...
function setupInactivity(){
  ...
  tickTimer = setInterval(() => {
    if(!usuario){ refs.sessionTimer.textContent = "sem sessão"; return; }
...
async function logHistory(registroId, action, beforeValue, afterValue, reason=null){
  ...
  }catch(e){ console.warn("Falha ao gravar histórico:", e); }
}

async function requirePermission(perm, actionForAudit){
  if(HorizonSecurity.hasPermission(usuario, perm)) return true;
  await logEvent("PERMISSION_DENIED", "denied", { permission: perm, action: actionForAudit || perm, view: currentView });
  alert("Acesso negado. Esta tentativa foi registrada na auditoria.");
  return false;
}

async function carregarUsuarios(){
  const {data, error} = await db.from(CONFIG.tabelaUsuarios).select("*").eq("ativo", true).order("display_name", {ascending:true});
  if(error){
    setStatus("Erro ao carregar usuários. Rode o SQL em supabase/001_security_static_schema.sql.", "error");
    console.error(error);
    return;
  }
  usuarios = data || [];
  refs.loginUsuario.innerHTML = usuarios.length
    ? usuarios.map(u => `<option value="${esc(u.id)}">${esc(u.display_name || u.username)}</option>`).join("")
    : `<option value="">Nenhum usuário cadastrado</option>`;
  refs.btnSetupAdmin.classList.toggle("hide", usuarios.length > 0);
  resumoLogin();
}

function resumoLogin(){
  const u = usuarios.find(x => x.id === refs.loginUsuario.value);
  if(!u){
    refs.loginResumo.innerHTML = "Nenhum usuário cadastrado. Crie um Admin Master inicial.";
    return;
  }
  refs.loginResumo.innerHTML = `<strong>Perfil:</strong> ${esc(u.role)}<br><strong>Status:</strong> ${u.locked_until && new Date(u.locked_until) > new Date() ? "bloqueado temporariamente" : "ativo"}<br><strong>Senha:</strong> hash seguro PBKDF2`;
}
...
async function login(e){
  ...
  if(u.locked_until && new Date(u.locked_until) > new Date()){
    await logEventRaw("LOGIN_BLOCKED", "denied", u, { login: u.username });
    alertLogin("Usuário ou senha inválidos.");
    return;
  }

  const ok = await HorizonSecurity.verifyPassword(senha, {
    hash: u.senha_hash,
    salt: u.senha_salt,
    iterations: u.senha_iteracoes
  });

  if(!ok){
    const count = Number(u.failed_login_count || 0) + 1;
    const patch = { failed_login_count: count, updated_at: nowISO() };
    if(count >= CONFIG.maxFalhasLogin){
      patch.locked_until = new Date(Date.now() + CONFIG.bloqueioMinutos * 60000).toISOString();
    }
    await db.from(CONFIG.tabelaUsuarios).update(patch).eq("id", u.id);
    await logEventRaw("LOGIN_FAIL", "denied", u, { login: u.username, failed_login_count: count });
    await carregarUsuarios();
    alertLogin("Usuário ou senha inválidos.");
    return;
  }

  await db.from(CONFIG.tabelaUsuarios).update({
  ...
  usuario = HorizonSecurity.setSession(u);
  await logEvent("LOGIN_OK", "success", { login: u.username });
  aplicarUsuario();
  hideLogin();
  await carregarRegistros();
}
...
async function setupAdminMaster(){
  const nome = prompt("Nome do Admin Master:");
  if(!nome) return;
  const login = prompt("Login do Admin Master:");
  if(!login) return;
  const senha = prompt("Senha do Admin Master:");
  if(!senha || senha.length < 4) return alert("Use uma senha com pelo menos 4 caracteres.");
  const hp = await HorizonSecurity.hashPassword(senha);
  const {error} = await db.from(CONFIG.tabelaUsuarios).insert({
    username: login.trim(),
    ...
    senha_hash: hp.hash,
    senha_salt: hp.salt,
    senha_algoritmo: hp.algorithm,
    senha_iteracoes: hp.iterations,
    created_at: nowISO(),
    updated_at: nowISO()
  });
  if(error) return alert("Erro ao criar Admin Master: " + error.message);
  await carregarUsuarios();
...
async function carregarRegistros(){
  ...
  let query = db.from(CONFIG.tabelaRegistros).select("*").order("created_at", {ascending:false});
...
function openMenu(){ refs.sideMenu.classList.add("show"); refs.shade.classList.add("show"); }
function closeMenu(){ refs.sideMenu.classList.remove("show"); refs.shade.classList.remove("show"); }
...
async function salvarRegistro(e){
  ...
  if(!refs.motivo.value || !refs.obs.value.trim()) return alertForm(["Preencha motivo e observação."]);
  ...
  let result;
  if(existente){
    result = await db.from(CONFIG.tabelaRegistros).update(payload).eq("id", existente.id).select().single();
  } else {
    result = await db.from(CONFIG.tabelaRegistros).insert({
      ...payload,
      status: "EM ABERTURA",
  ...
  if(result.error) return alertForm(["Erro ao salvar: " + result.error.message]);
...
async function encerrarRegistro(){
  ...
  const {data, error} = await db.from(CONFIG.tabelaRegistros).update(patch).eq("id", r.id).select().single();
  if(error) return alertForm(["Erro ao finalizar: " + error.message]);
...
async function reauthModal(title, message, includeReason=true){
  return new Promise(resolve => {
    ...
    refs.modalOk.onclick = async () => {
      ...
      const full = usuarios.find(u => u.id === usuario.id) || await getCurrentUserFull();
      const ok = await HorizonSecurity.verifyPassword(password, {
        hash: full?.senha_hash,
        salt: full?.senha_salt,
        iterations: full?.senha_iteracoes
      });
      if(!ok){
...
async function getCurrentUserFull(){
  const {data} = await db.from(CONFIG.tabelaUsuarios).select("*").eq("id", usuario.id).single();
  return data;
}
...
async function apagarRegistro(){
  ...
  const {data, error} = await db.from(CONFIG.tabelaRegistros).update(patch).eq("id", r.id).select().single();
  if(error) return alertForm(["Erro ao excluir: " + error.message]);
...
async function restaurarRegistro(){
  ...
  const {data, error} = await db.from(CONFIG.tabelaRegistros).update(patch).eq("id", r.id).select().single();
  if(error) return alertForm(["Erro ao restaurar: " + error.message]);
...
async function renderAuditoria(){
  ...
  const {data, error} = await db.from(CONFIG.tabelaAuditoria).select("*").order("created_at", {ascending:false}).limit(100);
  if(error){
...
async function renderHistorico(){
  ...
  const {data, error} = await db.from(CONFIG.tabelaHistorico).select("*").order("created_at", {ascending:false}).limit(80);
  if(error){
  ...
  refs.tbodyRegistros.innerHTML = `
    <tr><td colspan="11">
      <div class="auditItem">
        ...
        Exporta os registros, auditoria e histórico disponíveis pela chave pública. A restauração só aparece para Admin Master.
        <br><br>
